...
async def async_run_remote_tool(exchange, remote_agent_uid: str, tool_name: str, remote_tool_kwargs: Dict = None) -> ToolResult:
    print("Gonna run async call")
    logging.info(f"AgentMaster will execute tool {tool_name} on peer agent.")
    handle = Handle(AgentId(uid=remote_agent_uid), exchange=exchange)
...
async def async_run_tool(
    ...
    async with streamablehttp_client(mcp_url) as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            try:
                tool_result = ToolResult(**json.loads(actual_result))
                return tool_result
            except Exception as e:
                raise Exception(f"Could not get a real ToolResult from the following result:\n\n\n\t {actual_result}\n\n\nException: {e}")
